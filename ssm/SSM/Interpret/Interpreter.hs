...
interpret :: InterpretConfig -> T.Output
          ...
          outp <- execWriterT $ 
                  evalStateT (run >> emitResult) 
                      (interpState
                          ...
                          (boundEventQueueSize config)) -- bound on event queue
          return $ fromHughes outp

{- | Run the interpreter. This is quivalent to the @main@ function generated by the code
generator. -}
run :: Interp s ()
run = do tick
         c  <- now

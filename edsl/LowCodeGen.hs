...
compile_ wantMain tickLimit program = pretty 120 $ pprList compUnit
  where
    ...
    (decls, defns) = concat2 $ unzip $ map mkProcedure $
        Map.elems (funs program)
...
data TRState = TRState   -- | Procedure we are compiling
    { procedure :: Procedure
...
mkMain program =
  ...
  where
    ...
    refInit (ref, typ) = [ [citem|$ty:(svt_ typ) $id:ref;|]
                         , [citem|$id:(initialize_ typ)(&$id:ref);|]
                         , [citem|$id:ref.value = /* FIXME */ 0;|]
                         ]

    refPrint (ref, _ {-typ-}) =
        [citem|printf($string:fmtString, /* FIXME */ (long) $id:ref.value);|]
      where
...
mkEnter = do
    ...
    let act = act_ (name p)
        enter = enter_ (name p)
        step = stepId (name p)
        params = [cparams|$ty:act *caller, $ty:priority_t priority, $ty:depth_t depth|]
            ++ map param (arguments p)
    return ( [cedecl| $ty:act *$id:enter($params:params);|]
           , [cedecl|
...
mkCase (Wait ts) = do
    ...
    return $ fmap sensitizeTrig trigs
  ...
  where
    ...
    mkTrig refs (trig, _) = if trig `elem` refs
...
mkCase (Fork cs) = do
    ...
    refs <- map fst <$> gets locals
    caseNum <- nextCase
    let depthSub = (ceiling $ logBase (2 :: Double) $ fromIntegral $ length cs)
            :: Integer
    ...
    return $ zipWith mkCall [ 1 :: Int .. ] cs
        ++ [ [cstm| gen_act->pc = $int:caseNum; |]
           , [cstm| return; |]
           , [cstm| case $int:caseNum: ; |]
           ]

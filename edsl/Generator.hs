     ...
     where
       ...
       arbBody tab vars refs n = frequency $
           ...
           , (1, do b   <- arbitrary
                    ...
                    thn <- arbBody tab vars refs (n `div` 5)
                    els <- if b
                             ...
                             then Just <$> arbBody tab vars refs (n `div` 5)
                             else return Nothing
                    k   <- promote $ \() -> arbBody tab vars refs ((n `div` 5) * if b then 3 else 4)
           ...
           if null refs then [] else
             ...
             , (1, do delay <- sized $ arbExp TInt vars
                      r <- elements refs
       ...
       forkbdy (n, typs, bdy) vars refs = do
           ...
           undefined
           {- The Procedure constructor accepts a function (a -> b) as a component but
              we have no such function to give it here, and we can not supply a type
              annotation as we don't know what the type is. However, this function is never
              used in any of the passes I've implemented, so it can be removed
              from the constructor. -}
--           return $ Procedure n identity k
         where
